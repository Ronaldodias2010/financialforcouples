-- Atualizar função calculate_miles_for_transaction para suportar conversão automática de moeda
-- Exemplo: Cartão que dá pontos em USD, mas transação é em BRL → converte BRL para USD antes do cálculo
...
BEGIN
  ...
  IF NEW.type = 'expense' AND NEW.card_id IS NOT NULL THEN
    ...
    transaction_currency := COALESCE(NEW.currency, 'BRL');
        ...
        IF transaction_currency = 'BRL' AND rule_record.currency <> 'BRL' THEN
          ...
          IF rate_brl_to_target IS NOT NULL AND rate_brl_to_target > 0 THEN
            ...
            RAISE NOTICE 'Mileage: Converting BRL % to % = % (rate: %)', 
              NEW.amount, rule_record.currency, amount_in_rule_currency, rate_brl_to_target;
          ELSE
            RAISE WARNING 'Mileage: No exchange rate found for BRL -> %, using original amount', rule_record.currency;
          END IF;
          
        -- Caso 2: Transação em moeda estrangeira, regra em BRL
        -- Exemplo: Compra US$ 50, regra em BRL → converter US$ 50 para BRL
        ELSIF transaction_currency <> 'BRL' AND rule_record.currency = 'BRL' THEN
          ...
          IF rate_brl_to_target IS NOT NULL AND rate_brl_to_target > 0 THEN
            ...
            IF rate_brl_to_target IS NOT NULL AND rate_brl_to_target > 0 THEN
              ...
              RAISE NOTICE 'Mileage: Converting % % to % = % (via BRL)', 
                transaction_currency, NEW.amount, rule_record.currency, amount_in_rule_currency;
            END IF;
          ELSE
...
Suporta conversão de moeda: se a transação é em BRL mas a regra é em USD, 
converte o valor usando as taxas de câmbio armazenadas em exchange_rates.';