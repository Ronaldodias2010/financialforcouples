-- Atualizar função process_card_payment para criar transações de transferência
...
BEGIN
  ...
  -- Se pago via conta bancária, criar transações de transferência
  IF p_account_id IS NOT NULL THEN
    -- Debitar o saldo da conta
    UPDATE public.accounts 
    SET balance = balance - p_payment_amount,
        updated_at = now()
    WHERE id = p_account_id AND user_id = p_user_id;
    
    -- Criar transação de despesa (saída da conta)
    INSERT INTO public.transactions (
      ...
      'Pagamento de Cartão: ' || v_card_name, p_payment_date,
      'account_transfer', p_account_id, v_owner_user
    );
    
    -- Criar transação de receita (entrada virtual no cartão)
    INSERT INTO public.transactions (
      user_id, type, amount, description, transaction_date,
      payment_method, card_id, owner_user
    ) VALUES (
      p_user_id, 'income', p_payment_amount,
      'Pagamento de Cartão: ' || v_card_name, p_payment_date,
      'account_transfer', p_card_id, v_owner_user
    );
  END IF;

  -- Se pagamento via dinheiro, criar apenas uma transação de registro
  IF p_payment_method = 'cash' THEN
    INSERT INTO public.transactions (
      ...
      p_user_id, 'expense', p_payment_amount,
      'Pagamento de Cartão (Dinheiro): ' || v_card_name, p_payment_date,
      'cash', p_card_id, v_owner_user
    );
  END IF;
