-- 1. Modificar função process_card_payment para evitar duplicidade
...
BEGIN
  ...
  -- Registrar APENAS no histórico de pagamentos (sem criar transaction)
...
-- 2. Limpar transações antigas de "Pagamento de Cartão"
DELETE FROM public.transactions 
WHERE category_id IN (
  SELECT id FROM public.categories 
  WHERE LOWER(name) LIKE '%pagamento%cartão%'
  OR LOWER(name) LIKE '%pagamento%cartao%'
  OR LOWER(name) LIKE '%credit card payment%'
)
AND card_id IS NULL;