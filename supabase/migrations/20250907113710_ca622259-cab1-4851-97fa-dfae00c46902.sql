-- 1) Unificar categoria de receita "Investimentos" (remover duplicado singular) e padronizar nomes
DO $$
DECLARE
  keep_id uuid;
  dup_ids uuid[];
...
-- 1b) Limpeza de duplicados nas categorias de usuários (qualquer tipo), mantendo a com default_category_id definido e mais antiga
WITH ranked AS (
  SELECT id,
         ROW_NUMBER() OVER (
...
-- 2) Garantir existência da categoria padrão "Receita Extraordinária" (income)
DO $$
DECLARE
  rec_ext_id uuid;
BEGIN
  ...
  IF rec_ext_id IS NULL THEN
    ...
    VALUES (
      ...
      'gift',
      '#6366f1'
    ) RETURNING id INTO rec_ext_id;
  END IF;
END $$;

-- 3) Inserir/garantir tags para "Receita Extraordinária"
DO $$
DECLARE
  rec_ext_id uuid;
  tag_record RECORD;
...
BEGIN
  ...
  -- Para cada tag definida
  FOR i IN 1..array_length(tag_defs, 1) LOOP
    -- Criar tag se não existir (comparando por nome_pt normalizado)
    ...
    -- Relacionar tag com a categoria padrão (se ainda não relacionado)
    IF NOT EXISTS (
