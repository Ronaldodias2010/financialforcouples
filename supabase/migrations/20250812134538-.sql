-- Otimizar índices para reduzir consultas lentas identificadas
-- Adicionar índices compostos específicos para queries mais comuns

-- Índice para consulta de timezone (uma das mais lentas)
CREATE INDEX IF NOT EXISTS idx_timezone_names ON pg_timezone_names(name) WHERE name IS NOT NULL;

-- Índices específicos para realtime.list_changes otimização
-- Nota: Estes são índices do sistema, vamos focar nos nossos dados
