...
DROP FUNCTION IF EXISTS public.create_default_categories_for_user(uuid, text);

-- Recreate the function with fixed parameter naming
CREATE OR REPLACE FUNCTION public.create_default_categories_for_user(p_user_id uuid, user_language text DEFAULT 'pt'::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
